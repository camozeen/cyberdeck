...
const slug = 'iss-satellite-predictions';

const menuDisplay = 'ISS Passes';

const hoverDisplay = `
Prediction passes of ISS
`;
...
const getInstance = async (root) => {
  ...
  instance = blessed.list({
    parent: root,
    name: slug,
    label: ' {bold}{cyan-fg}ISS{/cyan-fg}{/bold} (PM2) ',
    tags: true,
    draggable: false,
    top: 0,
    right: 0,
...
const getPredictions = async () => {
  try {
    ...
    const response = await satpredict.issPredictions();
    log.log('got passes');
    return response;
  }
  catch (e) {
