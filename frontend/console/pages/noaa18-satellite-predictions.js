...
const slug = 'noaa18-satellite-predictions';

const menuDisplay = 'NOAA-18 Passes';

const hoverDisplay = `
Prediction passes of NOAA-18
`;
...
const getInstance = async (root) => {
  ...
  instance = blessed.list({
    parent: root,
    name: slug,
    label: ' {bold}{cyan-fg}NOAA-18{/cyan-fg}{/bold} (PM2) ',
    tags: true,
    draggable: false,
    top: 0,
    right: 0,
...
const getPredictions = async () => {
  try {
    ...
    const response = await satpredict.noaa18Predictions();
    log.log('got passes');
    return response;
  }
  catch (e) {
